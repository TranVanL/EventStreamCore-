...
TEST(TcpParser , parseValidframe) {
    ...
    EXPECT_NO_THROW(Event event = EventStream::parseTCP(frame); 
...
TEST(TcpIngestServer, EndtoEndFlow) {
    ...
    ssize_t bytes_sent = send(sock, frame.data(), frame.size(), 0);
    ASSERT_EQ(bytes_sent, frame.size()) << "Failed to send complete frame";
    close(sock);
    tcpServer.stop();
