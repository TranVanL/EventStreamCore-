...
void EventProcessor::init() {
    ...
    eventBus.subcribe([this](const EventStream::Event& event){
        spdlog::info("EventProcessor received event ID {} for processing", event.id);
        spdlog::info("TCP Event Payload Size: {}", event.payload.size());
        this->onEvent(event);
    });
}
    ...
    while (isRunning.load(std::memory_order_acquire)) {
        ...
        while (!incomingQueue.empty()) {
            ...
            try {
                if (workerPool) {
                    ...
                    workerPool->submit([ev, this](){
                        try {
                            ...
                            spdlog::info("Stored event ID {} from source type {}", ev.id, static_cast<int>(ev.sourceType));
                        } catch (const std::exception& e) {
                            spdlog::error("Failed to store event ID {}: {}", ev.id, e.what());
                        }
                    });
                } else {
                    storageEngine.storeEvent(ev);
                ...
                spdlog::error("Failed to schedule/store event ID {}: {}", ev.id, e.what());
            }
